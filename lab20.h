...
void Unit::equip(Equipment* _equipment)
{
	hpmax += _equipment->getStat()[0] - (equipment != NULL ? equipment->getStat()[0] : 0);
	atk += _equipment->getStat()[1] - (equipment != NULL ? equipment->getStat()[1] : 0);
	def += _equipment->getStat()[2] - (equipment != NULL ? equipment->getStat()[2] : 0);
	if (hp>hpmax) hp = hpmax;
	equipment = _equipment;
}

Equipment::Equipment(int _maxhp, int _atk, int _def)
{
	hpmax = _maxhp;
	atk = _atk;
	def = _def;
...
vector<int> Equipment::getStat() {return {hpmax, atk, def};}
